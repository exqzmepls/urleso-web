...
[`src/Urleso.Api.Client`](src/Urleso.Api.Client) is a hand-written [Refit](https://github.com/reactiveui/refit) client, kept in sync by hand with the vendored OpenAPI document ([`swagger.json`](src/Urleso.Api.Client/swagger.json)). See its [readme](src/Urleso.Api.Client/readme.md) for how to update the client after an API change.
